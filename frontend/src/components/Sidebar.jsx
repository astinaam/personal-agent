...
import { api } from '../api'
import { PanelLeft, Settings, Book, Plus, Trash2, LogOut } from 'lucide-react'

export default function Sidebar() {
  const [chats, setChats] = useState([])
  const { sidebarOpen, toggleSidebar, openPanel } = useUI()
  ...
  if (!sidebarOpen) {
    return (
      <div className="w-14 shrink-0 border-r border-border flex flex-col items-center py-3 gap-2">
    ...
    <div className="w-64 shrink-0 border-r border-border flex flex-col bg-surface">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        <div className="p-1.5 space-y-0.5">
          {chats.map(chat => (
            <div key={chat.id} className="group flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-surface-raised cursor-pointer transition-colors" onClick={() => navigate(`/chat/${chat.id}`)}>
          ...
          {chats.length === 0 && (
            <p className="text-xs text-neutral-600 text-center py-4">No chats yet</p>
          )}
        </div>
      </div>
